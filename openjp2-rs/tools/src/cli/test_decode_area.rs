use crate::getopt::{GetOpts, OptDef, ParsedOpt, PositionalArg};
...
fn parse_args(arguments: Vec<String>) -> Result<Args, String> {
  ...
  for opt in parser.parse_args(arguments.into_iter()) {
    match opt {
      ParsedOpt::Program(_) => {}
...
pub fn run_test_decode_area(args: Vec<String>) -> Result<(), String> {
