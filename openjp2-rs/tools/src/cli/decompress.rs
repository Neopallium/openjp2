use crate::{color::*, convert::*, params::*};
...
pub fn run_decompress(args: Vec<String>) -> Result<(), Box<dyn std::error::Error>> {
  // Parse command line options
  let (params, img_folder) = match parse_decompress_options(args)? {
    Some(opts) => opts,
    None => return Ok(()),
  };
